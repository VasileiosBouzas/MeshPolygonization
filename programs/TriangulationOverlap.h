...
inline bool check_overlap(Finite_faces_iterator it, std::vector<Triangle_2>* triangles) {
	...
	//if (std::abs(T.area()) <= 1e-12) { return false; } // Collinear!!!
	...
	if (area / std::abs(T.area()) >= 0.8) { return true; }
	return false;
}
...
inline std::vector<Segment_2> construct_alpha_shape(const Mesh* mesh, unsigned int id, Plane_3* plane, std::vector<Point_2>* points) {
	...
	std::vector<Halfedge> border = get_segment_border(mesh, id);
	...
	for (auto edge : border) {
		source = plane->to_2d(geom[mesh->source(edge)]);
		target = plane->to_2d(geom[mesh->target(edge)]);
		DT.insert_constraint(source, target);
	}
	...
	for (Finite_faces_iterator it = DT.finite_faces_begin(); it != DT.finite_faces_end(); ++it) {
		...
		if (check_overlap(it, &triangles)) { it->info().in_domain = true; }
	}
	...
	for (Triangulation_2::Finite_edges_iterator eit = DT.finite_edges_begin(); eit != DT.finite_edges_end(); ++eit) {
		...
		// If face in domain
		if (fh->info().in_domain) { ctr++; }
		// If neighboring face in domain
		if (fh->neighbor(eit->second)->info().in_domain) { ctr++; opposite = true; }
